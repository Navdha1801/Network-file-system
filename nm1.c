...
// Structure to represent an entry in the LRU cache
typedef struct
{
    char path[256];
    StorageServer *ss;
    time_t last_access_time; // Timestamp for LRU
} LRUCacheEntry;

// Structure to represent the LRU cache
typedef struct
{
    LRUCacheEntry entries[CACHE_SIZE];
} LRUCache;
...
void initializeLRUCache(LRUCache *lruCache)
{
    for (int i = 0; i < CACHE_SIZE; ++i)
    {
        strcpy(lruCache->entries[i].path, "");
        lruCache->entries[i].ss = NULL;
        lruCache->entries[i].last_access_time = 0;
    }
}

// Function to find an entry in the LRU cache
StorageServer *findLRUCacheEntry(LRUCache *lruCache, const char *path)
{
    for (int i = 0; i < CACHE_SIZE; ++i)
    {
        if (strcmp(lruCache->entries[i].path, path) == 0)
        {
            // Update the last access time
            lruCache->entries[i].last_access_time = time(NULL);
            return lruCache->entries[i].ss;
        }
    }
    return NULL;
}

// Function to update the LRU cache with a new entry
void updateLRUCache(LRUCache *lruCache, const char *path, StorageServer *ss)
{
    // Find an empty slot or the least recently used slot
    int emptyIndex = -1;
    int lruIndex = 0;
    time_t lruTime = lruCache->entries[0].last_access_time;

    for (int i = 0; i < CACHE_SIZE; ++i)
    {
        if (strcmp(lruCache->entries[i].path, "") == 0)
        {
            emptyIndex = i;
            break;
        }

        if (lruCache->entries[i].last_access_time < lruTime)
        {
            lruTime = lruCache->entries[i].last_access_time;
            lruIndex = i;
        }
    }

    int indexToUse = (emptyIndex != -1) ? emptyIndex : lruIndex;

    // Update the cache entry
    strcpy(lruCache->entries[indexToUse].path, path);
    lruCache->entries[indexToUse].ss = ss;
    lruCache->entries[indexToUse].last_access_time = time(NULL);
}
...
void serveClient(int client_socket, StorageServer storage_servers[], int num_servers,Hashmap hashmap,LRUCache lruCache)
{
    ...
    if (strcmp(client_request.operation, "READ") == 0 || strcmp(client_request.operation, "WRITE") == 0 || strcmp(client_request.operation, "GET_INFO") == 0)
    {
        ...
        // Try to find the entry in the LRU cache
                StorageServer *ss = findLRUCacheEntry(&lruCache, client_request.path);
        if (ss != NULL)
        {
            //updateLRUCache(&lruCache, ss.accessible_paths[j], &ss);
        ...
        else
        {
            ...
            ss = findStorageServerForPath(&hashmap, client_request.path);
            if(ss==NULL){
            ...
            send(client_socket, error_message, sizeof(error_message), 0);
             ...
             updateLRUCache(&lruCache, client_request.path, ss);
         // Send storage server details to the client
            send(client_socket, ss, sizeof(StorageServer), 0);
            printf("details sent are: %s %d %d\n", ss->ip_address, ss->port_nm, ss->port_client);
        }
...
if (last_slash != NULL) {
    ...
    strcpy(partial_path, client_request.path);
        ...
        if (ss != NULL)
        {
            ...
            // Log the confirmation
            logConfirmation(confirmation_message);
            insertIntoHashmap(&hashmap, client_request.path, ss);
            send(client_socket, confirmation_message, sizeof(confirmation_message), 0);
        }
        else
        {
...
if (last_slash != NULL) {
    ...
    strcpy(partial_path, client_request.path);
        ...
        if (ss != NULL)
        {
            ...
            // Log the confirmation
            logConfirmation(confirmation_message);
            //strcpy(ss->accessible_paths[ss->num_paths],client_request.path);
            //printf("%s\n",ss->accessible_paths[ss->num_paths]);
            insertIntoHashmap(&hashmap, client_request.path, ss);
            //ss->num_paths++;
            send(client_socket, confirmation_message, sizeof(confirmation_message), 0);
        }
    ...
    while (1)
    {
        ...
        if (registration_flag == 'S') // Storage Server registration
        {
            ...
            for (int j = 0; j < 100 && strlen(storage_servers[i].accessible_paths[j]) > 0; ++j)
            {
                ...
                updateLRUCache(&lruCache, storage_servers[i].accessible_paths[j], &storage_servers[i]);
            }
        ...
        else // Client request
        {
            ...
            // Serve the client
            serveClient(client_socket, storage_servers, num_servers, hashmap,lruCache);
        }
