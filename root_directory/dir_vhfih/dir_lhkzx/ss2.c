...
#define PORT_CLIENT 12363
#define PORT_NM 12395
#define MAX_FILE_CONTENT 1024
#define PORT_NM_PERSISTENT 7082
typedef struct {
    char ip_address[15];
    int port_nm;
} NamingServer;
...
void handleClientRequest(int client_socket, ClientRequest request)
{
    ...
    if (strcmp(request.operation+1, "READ") == 0) {
        ...
        FILE *file = fopen(request.path+2, "r");
        if (file != NULL) {
    //size_t bytesRead;
     //int count=0;
   // while ((bytesRead = fread(buffer, sizeof(char), sizeof(buffer), file)) > 0) {
    ...
    //}
        ...
        recv(client_socket, buffer, sizeof(buffer), 0);
        FILE *file = fopen(request.path+2, "w");
        if (file != NULL) {
    ...
    if (lstat(request.path+2, &file_stat) == 0) {
        if (S_ISDIR(file_stat.st_mode)) {
            // Handle directory information differently if needed
            // For example, list the contents of the directory
            // or send a message indicating it's a directory.
            // This is just an example, adjust it based on your requirements.
            char dir_message[] = "This is a directory.";
            send(client_socket, dir_message, sizeof(dir_message), 0);
        } else {
            // It's a regular file
